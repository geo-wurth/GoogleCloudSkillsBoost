...
gcloud container clusters create cluster-backend --machine-type=e2-medium --zone=$ZONE
gcloud container clusters get-credentials cluster-backend --zone=$ZONE
...
gcloud compute forwarding-rules create $FW_RULE_NAME --global --target-http-proxy=http-lb-proxy --ports=80